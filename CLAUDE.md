...
| **E** | Launch-Vorbereitung: Ressourcenlimits, Container-Healthchecks (S3-35), Log-Aggregation/Alerting (S3-36), Rechtstexte | ✅ vollständig — technische Seite seit 2026-08-05 komplett: Ressourcenlimits (S2-21), Migration-Fail-Fast (S3-34), getrennte Dev-Compose-Namen (S3-38), payloadfreier `GET /api/health` mit 503-Semantik + Container-HEALTHCHECKs (Z1/S3-35), Monitoring-Kette Uptime Kuma + Telegram + Backup VPS→NAS→OneDrive + Log-Rotation (S3-36, VPS-Härtung), CI-/Lieferkette (SHA-Pins, Dependabot, schlanke Docker-Kontexte, AGPL-3.0-LICENSE), Kuma-Monitor auf `/api/health` seit 2026-09-04. **Seit 2026-09-24 sind auch die Rechtstexte live:** Impressum und Datenschutzerklärung (de/en, #247) lesen operator-eigenes Markdown aus einem read-only gemounteten Verzeichnis (`Legal:ContentPath`, s. [docs/Operations.md](docs/Operations.md) „Legal pages"), die Löschfristen aus `RetentionPolicy` sind scharf geschaltet (`RETENTION_ENFORCE=true`, #244), und das Kontaktformular (#262, providerneutrales SMTP) ist die zweite elektronische Kontaktmöglichkeit nach § 5 DDG. **`robots.txt` bleibt bewusst gesperrt** (Betreiberentscheidung 2026-09-24, s. `PRODUCT.md` Brand Commitment 5 „Erklären statt verkaufen") — das Öffnen ist keine Launch-Voraussetzung mehr, sondern eine eigene, spätere Produktentscheidung. Offene Nachläufer bleiben in Epic #248: #243 (Selbstlösch-Knopf), #245 (Broadcaster-Selbstbereinigung), #260 (Harness respektiert die Ausschlussliste, erst nach dem verbindlichen Messlauf vom 2026-10-08) |
| **Review 2026-07-29** | 81 Befunde; Wellen A–E umgesetzt (D am 2026-08-02: Autorisierungs-Tests, Api-Filter-Matrix, Struktur-Test; E am 2026-09-24 mit den Rechtstexten abgeschlossen, `robots.txt` bleibt per Entscheidung zu) — s. [Review-2026-07-29-Umsetzung.md](docs/Review-2026-07-29-Umsetzung.md) | ✅ abgeschlossen |
...
- **`tests/EmotePurge.Worker.Tests`** ist bewusst container-frei und testet die puren Entscheidungs-/Zustandsklassen der Worker-Transporte (`TwitchReconnectBackoffPolicy`, `TwitchReconnectSignalSlot`, `TwitchWatchdogPolicy`, `SevenTvSubscriptionRegistry`, `SevenTvBackoffPolicy`, `EmoteUsageCounter`).
...
- **`EmotePurge.Worker`** — .NET Worker Service mit zehn Hosted Services: `Worker` (Boot-Recovery + Redis-Kommandos), `UsageFlushWorker`, `SevenTvPeriodicResyncWorker`, `SevenTvEventWorker` (7TV-EventAPI-WebSocket, Feature-Flag `SevenTv:EventApi:Enabled`), `TwitchConnectionWatchdog`, `WorkerHealthPublisher`, `WorkerRosterPublisher` (Per-Channel-Roster nach `worker:roster`, 60-s-Takt), `TwitchLivePollWorker` (Helix-Streams-Poll per App-Access-Token, `Twitch:LivePollIntervalSeconds` Default 300 s, schreibt `ChannelLiveDay`), `TwitchIdentityReconcileWorker` (hält Channel-Zeilen anhand der unveränderlichen Twitch-ID mit Helix in Sync, folgt Umbenennungen und führt Duplikate zusammen, `Twitch:IdentityReconcileIntervalMinutes` Default 60), `DataRetentionWorker` (setzt die Aufbewahrungsfristen aus `RetentionPolicy` durch — Tokens, inaktive Konten, beendete Abstimmungen, Audit-Log, deaktivierte Kanäle; wartet auf die Boot-Recovery plus `Retention:StartupDelayMinutes` Default 10, danach Takt `Retention:IntervalHours` Default 24; schreibt nur, wenn `Retention:Enforce` = `true` ist, Default `false` — der Trockenlauf loggt pro Durchlauf eine Warnung mit reinen Zählwerten) — und einen zweiten Einstiegspunkt `harness <kanal>` (kein Hosted Service, s. DECISIONS). `TwitchChatManager` kapselt genau **einen** langlebigen `TwitchLib.Client` für die gesamte Worker-Lebensdauer — **nie** pro Channel/Join neu instanziieren. „Einen" heißt seit 2026-09-08 *einen zur Zeit*: TwitchLib rekonnektiert selbst nicht mehr (`NoReconnectionPolicy`), jeder Verbindungsverlust ist ein Signal, und der Watchdog trägt die Wiederaufbau-Schleife allein — Ersatz des Objekts statt Reparatur, Warten auf den Handshake (`004`), danach ein gedrosselter Rejoin außerhalb der Lese-Schleife (Issue #68/#114, s. DECISIONS). Die Backoff-Entscheidung liegt TwitchLib-frei in `TwitchReconnectBackoffPolicy`, die Stale-Erkennung des Watchdogs (empfangene IRC-Frames inkl. Twitchs ~5-min-Server-PING, Schwelle 15 min — seit 2026-08-03, s. DECISIONS) ebenso TwitchLib-frei in `TwitchWatchdogPolicy`. Analog kapselt `SevenTvEventClient` genau **eine** EventAPI-Verbindung; dessen Subscription-Zustand liegt in `SevenTvSubscriptionRegistry` (Desired-State-first), das Reconnect-Pacing in `SevenTvBackoffPolicy` — beide pur und getestet.
- **`web/`** — Angular 22, Standalone Components + Signals (kein NgModule), Tailwind CSS, `@angular/cdk` fürs Virtual Scrolling, Transloco für i18n. `core/` = injectable Services, Guards, Models · `shared/` = wiederverwendbare Bausteine · `features/` = geroutete Seiten. Auth ist cookie-basiert wie das Backend, kein Token im Frontend. Details in [`web/.claude/CLAUDE.md`](web/.claude/CLAUDE.md). Die visuelle Sprache steht in [docs/UI-Designsprache.md](docs/UI-Designsprache.md) (verbindlich, handgeschrieben) und zusätzlich als maschinenlesbare Ableitung in [DESIGN.md](DESIGN.md) am Repo-Root — bei Widerspruch gilt die Designsprache.
...
7. **Die API liefert bei Fehlern nur sprachneutrale Codes** (`ApiErrorCodes`), nie fertigen Text. Ein neuer Code braucht denselben Eintrag in `web/src/app/core/i18n/api-error.ts` **und** in beiden Locale-Dateien. `api-error-locales.spec.ts` erzwingt seit 2026-08-02 die beiden hinteren Schritte (Code ↔ Übersetzung in beiden Sprachen); der Schritt von `ApiErrorCodes.cs` nach `api-error.ts` bleibt Disziplin — genau dort war die Liste vorher zweimal auseinandergelaufen.
...
12. **Neue Services/Guards/reine Utilities in `web/src/app/core/` + `shared/` bekommen einen co-located `*.spec.ts`** (Vitest; `HttpTestingController` für alles mit `HttpClient`, `TestBed.runInInjectionContext` für Guards). **Komponenten mit eigener, nicht-trivialer Entscheidungslogik ebenso — aber nur für diese Logik: Verhalten ja, Vorlage nein.** Rein darstellende Komponenten bekommen keinen Spec. **Erlaubt** sind Zustandsübergänge und `computed()`-Ergebnisse, Outputs und Dialog-Rückgaben, Sperrentscheidungen samt ihrem Grund, Accessibility-Semantik (Rolle, zugänglicher Name, `aria-*`-Bezüge) sowie eine Zeilen- oder Meldungsreihenfolge, **sofern sie anderswo als Vertrag dokumentiert ist** (z. B. `docs/UI-Designsprache.md` §7.2). **Nicht erlaubt** sind CSS-Klassen, Tailwind-Ketten, nicht-semantische Tags und Verschachtelung, Snapshots, und der Wortlaut einer Übersetzung als Selbstzweck — er darf eine Meldung identifizieren, er ist nicht der Prüfgegenstand. Faustregel im Zweifel: Was sich beim Umgestalten ändern darf, ohne dass ein Nutzer etwas anderes erlebt, gehört in keinen Spec; dafür bleibt Live-Testen im Browser das Mittel. Größere User-Flows über mehrere Seiten zusätzlich als Playwright-E2E mit gemockten `/api/**`-Responses.
...
- **Vor dem PR zusätzlich `node scripts/coverage-local.mjs`.** Die SonarCloud-Quality-Gate verlangt 80 % Coverage auf neuem Code und blockiert den Merge (`analyze` ist required check). Keine der drei Suiten oben misst Coverage — wer nur sie laufen lässt, erfährt erst am PR, dass das Gate reißt. Das Skript ist eine dateigenaue Näherung an Sonars zeilengenaue Messung und in beide Richtungen unscharf — es ersetzt den PR-Befund nicht, macht ihn aber vorhersehbar (s. [Tests](#tests)).
...
- **Twitchs JOIN-Limits — zwei getrennte, das härtere ist ein Bestandslimit.** (1) *Rate*: eine nicht-verifizierte Verbindung darf 20 JOINs pro 10 Sekunden; TwitchLib drosselt JOINs überhaupt nicht, aber seit 2026-09-08 (#68) laufen **alle** unsere JOIN-Pfade — auch der Rejoin nach einem Reconnect — über dieselbe 600-ms-Drossel außerhalb von TwitchLibs Lese-Schleife; die Rejoin-Dauer eines einzelnen Ereignisses wächst dadurch linear mit der Kanalzahl (0,6 s × N). (2) *Bestand*: **seit 2024-05-15 darf ein Account maximal 100 Chatrooms gleichzeitig gejoint haben** ([dev.twitch.tv/docs/chat](https://dev.twitch.tv/docs/chat/)) — ausgenommen sind nur Kanäle, in denen der Account Broadcaster oder Moderator ist. Die Messung vom 2026-07-30 (28 ungedrosselte JOINs in 5 s, 0 Fehler) hat die *Rate* auf dem inzwischen abgeschafften ungedrosselten Rejoin-Pfad geprüft und sagt über diese Decke nichts. Bindend sind seither Twitchs 100-Chatroom-Decke und die 7TV-EventAPI-Subscription-Grenze (docs/Konzept-Worker-Reconnect-2026-09-08.md, 7.2). Seit 2026-09-23 setzt die Api dem selbst eine Grenze: `Channels:MaxActiveChannels` (Default 80) blockt jeden Join, der die Zahl aktiver Channels über diese konfigurierbare Decke triebe, außer für globale Admins (s. DECISIONS). Vor einem größeren Ausbau: verifizierter Bot-Account (2.000 JOINs/10 s, hebt beide Limits) oder Sharding — Details in [Review-2026-07-29-Umsetzung.md](docs/Review-2026-07-29-Umsetzung.md). **Ein Wechsel auf EventSub ist ausdrücklich kein Ausweg**: Twitch wendet beide Limits auf `channel.chat.message` mit User-Token wortgleich an — s. [Untersuchung-Twitch-EventSub-2026-08-01.md](docs/Untersuchung-Twitch-EventSub-2026-08-01.md).
