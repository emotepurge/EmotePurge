...
public class ChannelEmoteSetObservationService(AppDbContext db) : IChannelEmoteSetObservationService
{
    public async Task RecordObservedSetAsync(string channelId, string emoteSetId, CancellationToken cancellationToken = default)
    {
        ...
        if (openInterval is null)
        {
            ...
            // interval and the IsBotActive flip in the *same* SaveChangesAsync. That atomicity gives
            // "IsBotActive = false implies no open row" — never the other way around, since a channel
            // that just joined and has not synced yet is active with no open row of its own — which is
            // exactly why this branch still rechecks IsChannelActiveAsync instead of trusting "no open
            // row" to mean inactive.
