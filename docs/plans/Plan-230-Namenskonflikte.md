...
Regel 4 „Name belegt", aliaslose Zeilen), `web/src/app/core/seven-tv/seven-tv-set-entries.ts`
...
| **Laufreihenfolge** | „in Quellreihenfolge" (T3) | `buildTransferPlan` gruppiert `replace → adoptSourceName → add → renameSource`, nie Quellreihenfolge | Ein Replace gibt mindestens so viele Einträge frei, wie sein eigenes ADD braucht; jedes Replace zuerst laufen zu lassen senkt die Spitzenbelegung des Ziel-Sets, nie hebt es sie — `transfer-plan.ts`'s eigene Doku |
| **Drift im Dialog** | nicht spezifiziert (nur „Ziel neu laden" als Banner-Aktion) | eine gedriftete Replace-Zeile legt sich selbst auf „Überspringen" zurück, und die Auflösungstabelle zeigt beim nächsten Öffnen sofort das Live-Gegenstück — ohne dass „Ziel neu laden" geklickt wird; der Vorschau-Loader selbst bleibt unverändert; ein fehlgeschlagener oder unvollständiger Read lässt Entscheidungen unberührt und gibt nichts frei | Betreiber-Entscheidung 2026-09-23: der Nutzer soll sehen, was jetzt wirklich im Ziel steht, bevor er erneut bestätigt — ein Reload-Zwang für etwas, das der Read schon weiß, wäre ein Umweg; ein unvollständiger Read kann für keine Zeile bürgen |
| **`defaultName`** | nicht in `TransferRowTarget` vorgesehen | eigenes Feld `defaultName: string \| null` — von `buildTransferPlan` immer `null` gesetzt, vom Bestätigungsdialog aus seinem eigenen Verifikations-Read gestempelt (`stampReplaceTargets`) | ein aliasloser Zieleintrag hat sonst keinen Namen, den ein Mensch lesen könnte — weder im Rückweg-Protokoll noch in einer künftigen Restore-Vorschau |
| **Dock-Anzeige der Löschmeldung** | im T7-Umfang belassen | Retry-Button, entfernte Anzahl und Drift-Anzahl (inklusive Alles-Drift-Fall) gehören dem Protokoll-Task (T7); Bildschirmleser-Spiegel sitzt im `DockOutcomeAnnouncer` | gleiche Begründung wie jede andere Dock-Zeile des Plans (§4.5): eine nur `aria-hidden` stehende Zeile braucht ihren gesprochenen Zwilling |
| **Restore-„Name belegt"-Zeile** | nicht festgelegt, welche Komponente sie zeigt | lebt in `mass-delete-panel.ts` plus dessen Announcer, nicht in `import-progress-section.ts` | Restore-Meldungen leben grundsätzlich im Panel, das den Restore-Lauf zeigt — Import und Restore sind getrennte Dock-Inhalte, auch wenn beide auf derselben Seite sitzen |
| **Regel 4 (Alias belegt)** | „Alias von anderer Id gehalten" ohne Einschränkung auf den Namenstyp | vergleicht nur **benannte** Aliase; ein Konflikt auf dem 7TV-eigenen Default-Namen ist ungeprüft und kann trotzdem in einem 409 enden | 7TVs Verhalten bei einem Default-Namen-Konflikt ist nicht live geprüft — die Regel bleibt auf das beschränkt, was tatsächlich gemessen ist |
