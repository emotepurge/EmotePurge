...
| **E** | Infra & Launch | ✅ abgeschlossen 2026-09-24 |
...
4. ~~Backup-Cronjob einrichten, siehe `docs/Backup-und-Restore.md`.~~ **Erledigt 2026-08-04** (VPS-Härtung): Cron 03:00 → NAS-Pull → OneDrive-Crypt, s. den Stand-Kasten in [Backup-und-Restore.md](Backup-und-Restore.md).
...
*Nachtrag 2026-09-16:* Die `public partial class Program;`-Zeile ist seit Commit `fa634e8` entfernt (Sonar/ASP0027 — .NET 10 erzeugt die Klasse per Source Generator auch ohne die explizite Deklaration). Das zweite Zugeständnis, `InternalsVisibleTo`, besteht unverändert fort. Details in `docs/DECISIONS.md`, Eintrag „`Program.cs` drops the explicit `public partial class Program;`" (2026-09-16).
...
**Welle E — Infra & Launch.** ~~S2-21 (Ressourcenlimits, vor dem Stresstest)~~ (2026-08-05 erledigt **und noch am selben Abend mit Realdaten kalibriert**: `deploy.resources.limits` 1.0 CPU / 512 M je für `api`/`worker` in beiden Compose-Dateien; Prod-Messung idle vs. 17k-Live-Channel zeigt nur ~10–15 MB Live-Delta — Werte bestätigt, der ursprünglich geplante Stresstest ist verworfen, s. DECISIONS-Eintrag „S2-21-Kalibrierung") → ~~S3-34~~ (2026-08-05 erledigt: `IPendingMigrationGuard` wirft beim Start von Api **und** Worker bei ausstehenden Migrationen — bewusst kein Auto-Migrate; `PendingMigrationGuardTests` gegen echtes Postgres) → ~~S3-38~~ (2026-08-05 erledigt, **abweichend vom Report-Vorschlag auf der Dev-Seite**: `docker-compose.yml` heißt jetzt `emote-purge-dev` mit Containern `emotepurge-dev-*`; die Prod-Datei blieb unangetastet, weil ein neuer Prod-Projekt-/Volume-Name das bestehende `postgres-data`-Volume verwaist hätte — die Api liefe nach dem Redeploy gegen eine leere DB) → ~~Z1-Aufteilung der Health-Endpoints + S3-35~~ (2026-08-05 erledigt: neuer payloadfreier `GET /api/health` — 200 nur bei `connected`, sonst 503, Policy `PublicHealth` —, `curl -f`-HEALTHCHECK im Api-Dockerfile, Datei-Heartbeat `/tmp/alive` + HEALTHCHECK im Worker-Dockerfile; s. DECISIONS-Eintrag) → ~~S3-36~~ (**2026-08-05 abgeschlossen**, in zwei Schritten: der externe Uptime-Check kam mit der VPS-Härtung am 2026-08-04 — Uptime Kuma auf dem NAS, 60-s-Takt, Telegram-Alerts, statt des vorgeschlagenen UptimeRobot. Der vorgeschlagene `healthchecks.io`-Push-Dead-Man's-Switch ist **verworfen**: Kuma ist vom VPS aus unerreichbar (Heimnetz); stattdessen wird der Kuma-Monitor auf `/api/health` umgestellt, dessen 503-Semantik den Pull zum Dead-Man's-Switch macht — **dieser Umstell-Handgriff in der Kuma-UI steht noch aus**. Log-Aggregation bewusst minimal geschlossen: Logs strukturiert, Rotation hostweit seit 2026-08-04, Zugriffsweg in infra-docs dokumentiert. Ebenfalls im Zuge der VPS-Härtung erledigt: die frühere „Offene Frage 15", Docker-Log-Rotation, s. u.**)** → ~~S3-37 (`pull_request`-Trigger)~~ (2026-07-30 erledigt, zusammen mit `paths-ignore` für Doku-only-Pushes — s. DECISIONS-Eintrag „CI: Doku-only-Pushes überspringen die Pipeline") → ~~S4-15~~/~~S4-16~~ (**2026-08-05 komplett**: alle Actions auf Commit-SHA gepinnt; Dependabot für github-actions/npm/nuget, Minor+Patch gruppiert — bewusst ohne `npm audit`-Gate und ohne NuGet-Cache, Begründung im DECISIONS-Eintrag. Der Format-/Lint-Teil war schon am 2026-08-01 erledigt: `prettier --check`, `eslint` und `dotnet format --verify-no-changes` gaten in `publish.yml`) → ~~S4-17~~ (2026-08-05 erledigt: `COPY src/ src/` statt `COPY . .`, tote `build`-Stage entfernt) → ~~S4-18~~ (**2026-08-05 komplett**: README-Teil war seit 2026-08-01 erledigt; LICENSE ist jetzt **AGPL-3.0** — Nutzerentscheidung, s. DECISIONS; das leere `marketing/` existiert nicht mehr, CONTRIBUTING für ein Solo-Repo bewusst verzichtbar) → ~~S2-20 (Rechtstexte)~~ (2026-09-24 live: Impressum und Datenschutzerklärung, Epic #248) → ~~`robots.txt` öffnen~~ (entfällt: Betreiberentscheidung 2026-09-24, die Datei bleibt bewusst gesperrt — s. DECISIONS). **Welle E ist damit abgeschlossen.**
...
Die restlichen (Stresstest-Messungen, 7TV-Editor-Permissions-Bitfeld, GHCR-Sichtbarkeit, Branch-Protection) sind in Abschnitt 10 des Reports unverändert nachlesbar — bis auf **Frage 17** („Cacht der Proxy Antworten?"), die seit dem 2026-08-05 beantwortet ist: die Api sendet jetzt `Cache-Control: no-store` auf allem unter `/api/` (Header-Middleware in `Program.cs`, Tests in `ApiCacheControlTests`). Die **Docker-Log-Rotation** (Frage 15) ist seit dem 2026-08-04 beantwortet: hostweit per `/etc/docker/daemon.json` (`json-file`, 10 MB × 3 Dateien, VPS-Härtung — Bericht im privaten Repo `sensitron/infra-docs`); greift je Container erst beim nächsten Redeploy.
