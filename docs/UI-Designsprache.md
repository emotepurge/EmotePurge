...
- **Selection-*creating* commands** — a control whose whole purpose is to produce a selection out of nothing (mark-all) — belong neither in the header nor in the dock. The dock does not render at zero selection (`actionDockHasContent`), so it cannot host the one control that would put something into it; the header's commands, by contrast, are page-wide and carry no notion of "the current view" to mark. It sits with the controls that define that view instead — the filter row, beside the reset control — carries the count in its label since nothing beside it already states one, and is disabled once the view it would act on is already fully marked (docs/DECISIONS.md 2026-09-19, "mark all" on the usage toolbar).
