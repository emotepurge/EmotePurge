...
| E15 | Nachweis, dass `x-ratelimit-search-remaining` bei den Bestenlisten-Stichproben über 90 bleibt | **Umgesetzt und vom Betreiber am 2026-09-14 entschieden:** das eine maßgebliche Log-Ereignis je Upstream-Request gehört dem **Service** (`SevenTvLeaderboardService.LogUpstreamRequest`), nicht dem Client — für `SearchEmotesAsync` schreibt der Client **keine** Zeile. `SevenTvEmoteSearchPageResult` trägt `RateLimitLimit`/`RateLimitRemaining`/`RateLimitReset` stattdessen in **jedem** Ausgang (Erfolg, `RateLimited`, `Unavailable`), damit der Service sie unabhängig vom Ausgang für seine eine Zeile zur Hand hat. Der Nachweis selbst bleibt wie geplant: **aus den Log-Zeilen des Abnahmelaufs, eingegrenzt auf die Stichproben der Bestenliste** — nicht aus einem gespeicherten Minimum im `RateLimitTelemetryStore` und ausdrücklich **keine** Aussage über das Eimer-Minimum aller Verbraucher | Zwei Zeilen je Request — eine im Client, eine im Service — hätten AK 31s Zählung mehrdeutig gemacht: der Client kennt das Budgetfenster nicht und könnte `usedInWindow` nicht mitschreiben, eine zweite, unvollständige Zeile wäre also entweder Duplikat oder Lücke gewesen. Der Store hält nur `LastHeaderSample` (F5). Der Eimer erneuert sich pro ~60 s, verglichen wird also pro Minute, nicht pro Stunde: im ungünstigsten Fall feuert unser Budget alle 10 Freigaben innerhalb einer Minute, das sind 10 von 100 — „10 von 6.000 pro Stunde" war der falsche Maßstab. Das Minimum über alle Verbraucher gehört zur gemeinsamen Bremse (E1, Abschnitt 16); die Vorab-Kontrolle aus AK 36 stellt unterdessen ohne Code sicher, dass keine hängende Kanalzeile den Eimer mitzieht — bei der heutigen Kanalzahl Hygiene, kein akutes Kontingentrisiko (E1) |
...
| Unerwartete eigene Ausnahme (kein von 7TV gemeldeter Zustand, kein Breaker-/Budget-Zustand) | Backstop — kein Ergebnispfad hat sie abgefangen | **500** `unexpected_error` über den API-weiten `UseExceptionHandler` (`Program.cs:208-212`), **kein** 503. Der Vorrat-Eintrag faultet dabei (`SevenTvLeaderboardStore` behandelt einen `Faulted`/`Canceled`-Task als sofort abgelaufen, Abschnitt 6) und wird beim nächsten Leser ersetzt; der Deckel selbst bleibt heil |

**Bewusst kein vierter 503-Arm für den eigenen Fehler.** Im Review ausdrücklich als richtig bestätigt:
eine unerwartete Ausnahme reist zum Aufrufer und endet als 500, nicht als 503. Ein `catch`, das sie
nachträglich doch noch in eines der drei `foreign_channel_seventv_unavailable`-Ergebnisse übersetzt,
würde einen eigenen Bug hinter „7TV ist gerade nicht erreichbar" verstecken — genau deshalb steht
diese Zeile hier, statt dass die Zuordnung nur aus dem Fehlen eines vierten Codes erschlossen werden
muss.
...
| Alarm | **Warnung bei ≥ 6 Requests im 60-min-Fenster**, einmal je Fenster, **vor** dem Deckel. 6 = Erwartung 4 plus ein fehlgeschlagener Füllzyklus innerhalb eines Prozesslaufs (ein solcher Zyklus kostet 2 Requests, s. Füllpfad). Ein Doppel-Fill bei Ablauf ergäbe 8 und löst den Alarm aus. Der Alarmzähler zählt **je Prozesslauf** (er liest `usedInWindow` desselben In-Process-Budgets) und **vergisst Anfragen vor dem Neustart** — Requests mehrerer Läufe in derselben Stunde sieht er nie zusammen; eine Absturzschleife macht der Container-Healthcheck sichtbar, nicht dieser Alarm. **Gegatet auf gewährte Freigaben** (`granted == true`): die Warnung läuft nur nach einem erfolgreichen `TryCharge`, weil eine Verweigerung `usedInWindow == MaxRequests` meldet und die Schwelle sonst auf Anfragen zöge, die nie stattfanden. Im Review festgestellt: der Unterschied zwischen Gate und Nicht-Gate ist für **jede** Konfiguration unbeobachtbar, weil jeder Weg zu einer Verweigerung über eine gewährte Freigabe führt, die den Einmal-je-Fenster-Riegel schon gesetzt hat — entschärft wird die Sache also vom **Riegel**, nicht vom Gate. **Fällt der Riegel je weg** (etwa: Warnung je Request statt einmal je Fenster), wird das Gate tragend und ist ungetestet |
...
| `ListSelection<ForeignEmoteRow>` | Einzelauswahl, Shift-Bereich | **Belegt, korrigierter Pfad:** unverändert; die Auswahl-über-Listenwechsel-Falle wird durch Neuinstanziierung im Step umgangen, nicht in der Klasse gelöst (`web/src/app/shared/selection/list-selection.ts:39-42` — die Datei liegt unter `shared/selection/`, nicht unter `shared/seven-tv/` wie die Nachbarbausteine dieser Tabelle) |
...
| `ForeignEmoteSetCache`, `ForeignEmoteSetRequestCoalescer` | werden **nicht** gebraucht | **Belegt, korrigierte Begründung:** nur `ForeignEmoteSetRequestCoalescer` ist auf `ForeignEmoteSetLookupResult` typisiert (`ForeignEmoteSetRequestCoalescer.cs:32`); `ForeignEmoteSetCache` ist auf `ForeignEmoteSet` typisiert (`ForeignEmoteSetCache.cs:27`, `:49`), nicht auf `ForeignEmoteSetLookupResult` — die Ergebnisse laufen also nicht beide durch denselben Typ. Beide sind trotzdem nicht anschlussfähig: der eine cacht Set-Vorschauen nach Kanalname, der andere koalesziert Kanal-Lookups — keiner kennt Sortier-Schlüssel oder Seiten. Der Vorrat koalesziert selbst und ist in-process (Entwurf). Kein Umbau, nur die alte Begründung war falsch |
...
| `RateLimitTelemetryStore` | zählt die neue Call-Source ohne Änderung | **Belegt, korrigierter Beleg:** dimensioniert über `(provider, callSource)`-Strings, keine Allowlist im Store — Bildung in `ProviderDimension(...)` (`RateLimitTelemetryStore.cs:347-348`), aufgerufen aus `RecordProviderResponseAsync` (`:120`), **nicht** bei `:174-175`: diese Zeilen liegen in `ReadAsync` und summieren dort nur Buckets über `SumBucketsAsync`, ohne eine Dimension zu bilden. +1 Integrationsfall bestätigt es |
