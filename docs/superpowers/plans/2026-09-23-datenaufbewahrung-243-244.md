...
`Channel.DeactivatedAtUtc`) machen die Fristen messbar; eine Migration legt sie an und setzt den
Kanalstempel für Bestandszeilen auf den Migrationszeitpunkt. Ein neuer Core-Vertrag
`IAccountDeletionService` löscht ein Konto atomar (Stimmen → Pseudonymisierung → Zeile) und wird
vom Admin-Endpoint wie vom Job gerufen. Ein zweiter Vertrag `IDataRetentionService` führt einen
Durchlauf über die fünf Kategorien aus und liefert Zählwerte; ob er schreibt oder nur zählt, sagt
`Retention:Enforce` (Default `false`). Ein zehnter Hosted Service `DataRetentionWorker` ruft ihn
nach dem Boot-Gate und dann alle 24 h.
...
1. **„Letzte Nutzung" ist heute nicht messbar.** `User.LastLogin` wird ausschließlich im
   ...
   *sliding* (`Program.cs`): wer wöchentlich vorbeischaut, loggt sich nie neu ein — „12 Monate
   ...
   `lastActivity = max(LastLogin, LastSeenAtUtc ?? LastLogin)`.
   Backfill: keiner nötig — `null` fällt auf `LastLogin` zurück, das für jede Zeile gesetzt ist.
...
8. **Muster für den Job:** `TwitchIdentityReconcileWorker` — wartet auf `BootRecoveryGate`,
   ...
   Services (heute 9).
9. **Migration:** eine, additiv, zwei nullable Spalten plus das Backfill-`UPDATE` aus Befund 2.
   Prod-Migration manuell per Tunnel **vor** dem Deploy (CLAUDE.md „Prod-Migration"); das alte
   Image ignoriert die Spalten.

## Verträge

**`IAccountDeletionService` (Core)** — `DeleteAsync(twitchUserId, AuditActor actor,
AccountDeletionReason reason, ct)` → Ergebnis `Deleted | NotFound`, dazu die Zählwerte
(gelöschte Stimmen, pseudonymisierte Audit-Einträge). `AccountDeletionReason` ist ein Enum
(`AdminRequest`, `Inactivity`); der Wert landet im Audit-Detail. Der Actor ist Pflicht (Admin
aus dem Principal, `AuditActor.System` aus dem Job).
...
gelöscht, Kanäle gelöscht, Kanäle nachgestempelt) plus `Enforced`-Flag. Im Trockenlauf sind die
Zähler die `COUNT`-Ergebnisse derselben Prädikate, die der scharfe Lauf löschen würde — **ein**
Prädikat je Kategorie, in einer Methode formuliert, von beiden Modi benutzt, damit die gezählte
und die gelöschte Menge nicht auseinanderlaufen können.
...
**`IChannelService`** — neue Methode `PurgeIfInactiveSinceAsync(channelName,
DateTime deactivatedBeforeUtc, AuditActor actor, ct)` → `Purged | NotFound | StillActive`. Der
Retention-Zwilling von `PurgeAsync`: gleicher Audit-Eintrag `channel.purge` mit Detail
`{ reason: "retention" }`, aber **bedingtes** Löschen in einem Statement (`IsBotActive = false AND
DeactivatedAtUtc < cutoff`), damit ein Join, der zwischen Auswahl und Löschung dazwischenkommt,
gewinnt statt gelöscht zu werden. Kein LEAVE-Publish: der Worker ist in einem inaktiven Kanal
nicht drin, und hat ein Join ihn gerade aktiviert, trifft das Statement keine Zeile.
...
1. Zeile laden; fehlt sie → `NotFound`, nichts geschrieben, kein Audit-Eintrag (No-op-Regel).
...
7. Audit-Eintrag `user.delete` mit dem **Actor des Auslösers**, `TargetType = "user"`,
   **`TargetId = Marker`** und Details `{ reason, votesDeleted, auditEntriesPseudonymised }` —
   ohne ID, ohne Login. Der Nachweis „am Tag X wurde ein Konto auf Anfrage gelöscht" reicht; die
   E-Mail-Anfrage hält der Betreiber außerhalb der App. Denselben Eintrag mit Identität zu
   schreiben, hieße, den Marker in derselben Transaktion wieder zu unterlaufen.
8. Commit, **danach** `IModRoleCache.InvalidateUserAsync(id)` — Redis kann nicht in der
   Transaktion sein; schlägt es fehl, verfallen die Einträge per TTL (10 min bis 25 h) von selbst.
   Der In-Process-Zustand von `TwitchTokenRefreshGate` bleibt; er ist ein Semaphor je ID ohne
   Nutzdaten.

**Idempotenz:** zweiter Aufruf → `NotFound`. **Nebenläufigkeit:** Admin-Löschung und Job-Löschung
desselben Kontos → einer findet keine Zeile. Ein Vote, der zwischen Schritt 3 und 6 einschlägt,
lässt Schritt 6 an der FK scheitern → Rollback → der Aufrufer bekommt den Fehler (500 bzw. der
Job zählt den Fehlschlag und versucht es beim nächsten Tick). Kein Serializable nötig.
**Re-Login nach Löschung:** neue Zeile mit derselben Twitch-ID, `LastLogin = now` — die
pseudonymisierten Einträge lassen sich nicht rückverknüpfen, weil der Marker die ID nicht mehr
trägt.
...
- **Ein Scope je Tick, `IDataRetentionService.RunAsync(options.Enforce)`,** Log am Ende:
  Information im scharfen Modus, **Warning im Trockenlauf** („retention dry run: N/N/N/N/N
  would be affected, Retention:Enforce is false") — die Warnung ist der Erinnerungsmechanismus
  gegen ein für immer vergessenes `false`. Jeder Tick loggt, auch bei lauter Nullen: er ist der
  Lebensbeweis des Jobs.
- **Kategorien in fester Reihenfolge, je eigene Transaktion(en):**
  ...
  2. Konten: Kandidaten `lastActivity < now − 365 d`, höchstens `MaxAccountsPerRun` je Tick,
     je Konto `IAccountDeletionService.DeleteAsync(id, AuditActor.System, Inactivity)` — eigene
     Transaktion je Konto, Fehlschlag eines Kontos bricht die Kategorie nicht ab.
...
- **Nebenläufigkeit Api ↔ Worker:** Alle Löschprädikate sind bedingt und atomar (Statement mit
  WHERE), Konten laufen über den gemeinsamen Pfad. Ein `LastSeenAtUtc`-Stempel, der zwischen
  Auswahl und Löschung eines Kontos fällt, ist der eine verbleibende Race — Fenster Millisekunden,
  Fall „nach 365 Tagen genau in dieser Sekunde zurück"; akzeptiert und im DECISIONS-Eintrag genannt.
...
- **Kanal tritt nach Leave wieder bei:** `CompleteJoinAsync` nullt `DeactivatedAtUtc`; innerhalb
  ...
  eine neue Zeile an — `TrackingResumedAt` ist dann `null`, `CreatedAt` ehrlich.
...
- **Nutzer gerade eingeloggt:** nächster Request → abgewiesen (Befund 5). Löscht sich ein Admin
  selbst über die Admin-Liste, ist das erlaubt (Präzedenz Revoke) und der Dialog sagt es
  (`selfHint`-Muster).
...
- [ ] Migration `AddRetentionTimestamps`: zwei Spalten, Backfill-`UPDATE` für inaktive Kanäle
      auf `now()` (Begründung Befund 2 im Migrationskommentar).
...
- [ ] DECISIONS: „Two retention timestamps: `LastSeenAtUtc` and `DeactivatedAtUtc`, and why the
      channel backfill starts the clock at migration time" (kann mit T2 zusammengelegt werden,
      wenn derselbe Commit).
...
- [ ] `IAccountDeletionService`, `AccountDeletionReason`, `AuditActions.UserDelete`,
      `AuditActor.DeletedUser`; Implementierung nach „Kontolöschpfad im Detail"; Registrierung.
- [ ] Tests (Integration, Postgres + Redis-Fixture): Stimmen weg, Zeile weg, Audit als Actor
      ...
      `user.delete`-Eintrag ohne Identität, `NotFound` beim zweiten Aufruf, Rollen-Cache geleert,
      offene Session verliert die Stimme und der Score sinkt (über `IVoteSessionQueryService`).
...
- [ ] DECISIONS: „Account deletion: votes go, audit entries are pseudonymised, the deletion entry
      carries no identity".

### T4 — Admin-Endpoint und Admin-UI (`sonnet`)

- [ ] `DELETE /api/admin/users/{twitchUserId}` (Muster `revoke-sessions`), Actor aus Principal,
      `AccountDeletionReason.AdminRequest`.
...
### T5 — Retention-Service (`opus`)

- [ ] `RetentionPolicy`, `IDataRetentionService`, `RetentionRunSummary`, `RetentionOptions`
      (+ `Validate`, Registrierung, `TimeProvider`-Registrierung); `IChannelService.
      PurgeIfInactiveSinceAsync`; Implementierung nach „Der Job" (Kategorien, Prädikate einmal,
      Batches, Transaktionsgrenzen).
- [ ] Tests (Integration): je Kategorie ein „knapp drüber"/„knapp drunter"-Paar gegen die fixe
      Uhr; Trockenlauf zählt identisch und **schreibt nichts** außer Nachstempeln; Nachstempeln
      macht eine Zeile beim selben Lauf nicht fällig; `PurgeIfInactiveSinceAsync` trifft keine
      inzwischen aktive Zeile und schreibt dann keinen Audit-Eintrag; offene Session bleibt;
      Konto-Kappe `MaxAccountsPerRun`; Fehlschlag eines Kontos stoppt die anderen nicht.
- [ ] DECISIONS: „Retention periods as code constants, one predicate per category for count and
      delete, and the dry-run default".

### T6 — Hosted Service, Konfiguration, Compose (`sonnet`)
...
### T7 — Dokumentation für Betreiber (`sonnet`)

- [ ] `docs/Operations.md`: Abschnitt „Data retention" (englisch) — Tabelle der Fristen mit
      Verweis auf `RetentionPolicy`, der Trockenlauf, wie man scharf schaltet, wie die Log-Zeile
      aussieht, die Admin-Kontolöschung als Weg für E-Mail-Anfragen.
...
### T8 — Gates (Orchestrator, kein Subagent-Task)
...
3. **Fixtures** per SQL in der Wegwerf-DB: je Kategorie eine fällige und eine nicht fällige
   ...
   Session, Audit-Einträge als Actor und als Target. Erwartung: Trockenlauf loggt die Zählwerte
   und ändert nur den Stempel; scharfer Lauf löscht exakt die fälligen Zeilen; Audit-Marker sitzen.
4. **Api aus dem Worktree** auf `:5151` (nur, wenn dort nichts läuft; die Twitch-Redirect-URI ist
   ...
   Testkonto, das der Admin löscht.
...
1. **„Letzte Nutzung" über `LastSeenAtUtc` (täglich gestempelt) statt `LastLogin` allein.**
   Empfehlung: ja — ohne den Stempel löscht die 12-Monats-Regel Nutzer, die täglich da sind
   (Befund 1). Kostet eine `UPDATE`-Zeile je Nutzer und Tag.
2. **Der `user.delete`-Audit-Eintrag trägt keine Identität** (nur Grund und Zählwerte).
   Empfehlung: ja — sonst unterläuft der Eintrag die Pseudonymisierung derselben Transaktion; der
   Nachweis der Anfrage liegt beim Betreiber in der E-Mail.
3. **Keine Audit-Einträge je gelöschter Session/je geleertem Token**, nur der Kanal-Purge
   ...
   Retention-Zeilen im Jahr würden es fluten. Der Job loggt Zählwerte.
...
5. **Kanal-Backfill auf Migrationszeitpunkt** statt aus dem Audit-Log rekonstruiert.
   Empfehlung: Migrationszeitpunkt — Sicherheitsnetz vor Genauigkeit; die Differenz ist Wochen.
6. **Kein Admin-Sonderfall bei Inaktivität.** Empfehlung: keiner — Rechte hängen an der
   Konfiguration, nicht an der Zeile.
7. **Selbstlöschung eines Admins über die Admin-Liste erlaubt** (Revoke-Präzedenz, Hinweis im
   Dialog). Empfehlung: erlauben — ein Verbot bräuchte einen neuen Fehlercode für einen Fall, der
   harmlos ist.
8. **`MaxAccountsPerRun = 100`** als Kappe je Tick. Empfehlung: ja — begrenzt die Laufzeit des
