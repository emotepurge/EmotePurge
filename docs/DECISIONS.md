...
### 2026-09-10 — The 7TV write surface moves to v4, because v3 no longer accepts umlaut aliases (#149)

**Betrifft:** `web/src/app/core/seven-tv/seven-tv-run-engine.ts` · `web/src/app/core/seven-tv/seven-tv-import.service.ts` · `web/src/app/core/seven-tv/seven-tv-restore.service.ts` · `web/src/app/core/seven-tv/seven-tv-delete.service.ts` · `web/src/app/shared/seven-tv/import-preview.ts` · `web/src/app/shared/seven-tv/already-present-filter.ts` (new) · `web/src/app/shared/seven-tv/{import-flow,restore-flow,mass-delete-panel,import-progress-section}.ts` · `web/src/app/core/http/api-auth.interceptor.ts` · `web/e2e/support/mocks.ts` · `web/e2e/emote-import.e2e.spec.ts` · `web/public/i18n/{de,en}.json` · `docs/Architectur.md` · `docs/plans/Plan-149-7TV-v4-Schreibflaeche.md`

**The first live import from a foreign channel failed for 4 of 332 emotes, all for the same reason: their alias contains umlauts.** `UnterschätzDasMalNicht`, `Sitzgemüse`, `Löwe` and `Gänsehosen` came back with `Failed to parse "String": invalid emote name`. The run itself was clean — 328 copied, 4 reported, nothing lost silently — but those four were fundamentally uncopyable over the old path. 7TV v3 guards the `name` argument of `emoteSet(id).emotes(id, action, name)` with a validator whose regex is `^[a-zA-Z0-9_\-():!+|.'?><\p{Emoji_Presentation}*$#]{1,100}$` — ASCII plus emoji. What it rejects is therefore not the umlaut but the whole non-ASCII class: accents, Cyrillic, CJK, `ß`.

**The obvious workaround does not hold, and that could be established before building anything.** The ticket proposed a two-step copy: add with the global base name first, then rename to the alias. But in v3 adding and renaming are **the same field with the same argument and the same validator** — only the `action` enum tells them apart. Step two would have failed at exactly the same barrier. Dropping the argument and letting 7TV fall back to the base name is no better: in the reference channel's set **all 27** aliases containing an umlaut or `ß` have a differing, ASCII-only base name (`Gänsehosen`→`Gaensehosen`, `wutköder`→`rageBait`, `SchrödingersChat`→`CrossEyesBob`). That would have silently written the wrong name — worse than a reported failure.

**Umlaut aliases nevertheless exist, because the rule was tightened after the fact.** It originally read `\w`, and `\w` is Unicode-aware in Rust's `regex`. Commit `17676a62` of 2025-01-27 narrowed it to `a-zA-Z0-9_`. All four entries predate that (2024-01 through 2024-11). So 7TV tolerates them; it does not permit them.

**v4 fixed precisely this, and v3 was never brought along.** The upstream pull request that split set-level aliases onto their own Unicode-aware validator (merged 2025-12-01, referenced in `docs/plans/Plan-149-7TV-v4-Schreibflaeche.md`) left the canonical emote name strict and only loosened the alias. v4 also dropped the `action` enum and gives each operation its own field, carrying the emote in an `EmoteSetEmoteId { emoteId, alias }` input object. The fix is therefore **one** call rather than two: no intermediate step, no partial state, no extra mutation and so no additional cost on large sets.

**The permitted character set is measured, not transcribed.** On 2026-09-10, 25 aliases were probed live against a disposable set. Accepted: letters of any script, `ß`, emoji, mixed forms such as `Übergröße😀Träger`, the characters `:` `'` `&`, and a 100-character alias. Rejected with `invalid emote alias`: whitespace in any position, tab, newline, zero-width space, `/` `\` `"` `,` `;` `@` `%` `=` `[` `]` `{` `}` `~` `^`, 101 characters, and the empty string. That agrees point for point with the regex in 7TV's source — two independent directions, no contradiction. The differing error text (`invalid emote alias` rather than `invalid emote name`) is at the same time the proof that the two validators really are deployed separately.

**The client-side preview warned about exactly what now works.** `isNameRejectedBySevenTv` classified every alias with a codepoint above `0x7f` as hopeless, and the confirmation dialog announced those rows to the user as certain failures. Without that file #149 would have been half a fix. The comment above it also claimed the opposite of the measurement ("the alias is not affected — 7TV does allow umlauts there"); the two live rejections it cited as evidence were themselves alias rejections. The check is now a **blocklist of observed rejections**, deliberately not an allow-list transcribed from the regex: Rust's Unicode-aware `\w` and `\p{Emoji_Presentation}` have no faithful JavaScript equivalent, so a transcription would be its own source of error. The error direction favours it too: a blocklist under-warns, which costs one reported failed row, whereas an allow-list over-warns and talks the user out of an emote that would have worked. That was the bug this entry is about.

**The rate-limit contract is identical, yet the quota stays learned.** v4 uses the same `ApiError` module as v3: the same guard on every set mutation (`RateLimitGuard::new(RateLimitResource::EmoteSetChange, 1)`), the same `emote_set_change` bucket, one ticket per call, code `RATE_LIMIT_EXCEEDED`, `extensions.status: 429`, and the reset values still mirrored into `extensions.headers`. Confirmed live; rejected rows cost no ticket. `isRateLimitError`, `readRateLimitInfo` and `parseHeaderNumber` are therefore unchanged, and the rate-limit helper in the delete spec survived byte-for-byte — that is the evidence rather than the claim. **What does not follow:** that the quota is fixed. The entry of 2026-08-01 still stands — `RateLimits` lives per role in 7TV's database and can change without notice. The measured value (100/60 s) proves one role at one moment. The learning mechanism stays.

**The abort decision stops guessing.** v4 reports missing write permission as **HTTP 200** with `extensions.code = "LACKING_PRIVILEGES"` and `extensions.status = 403`. `abortsForMissingPrivileges` previously tested `httpStatus` 401/403 **and** the substrings `insufficient privileges` / `missing permission` — a list its own comment flagged as "as far as it is known from their frontend". After the move neither condition would have matched: the transport succeeds, so `httpStatus` is `null`, and neither substring occurs. An import without write permission would have run through **every** row and burned the rate-limit budget. The run engine therefore carries `extensions.code` through to the `abortOn` hook, and the substring list is gone with no replacement. The `httpStatus` 401/403 test remains: an invalid token still yields a real `HTTP 401` whose body sits **outside** the GraphQL schema (`{"status":"Unauthorized","error_code":1000,"error":"invalid session"}`) and offers no code to read.

**Duplicate protection belongs in the same change, because the migration is what makes it necessary.** 7TV's `addEmote` does not deduplicate by emote id: it only checks whether the *alias* collides (then `BAD_REQUEST`/409) and otherwise blindly appends another entry — an emote already in the set under a different alias ends up in it twice. Observed live: 77 → 79 entries on one additional call. On v3 that path was unreachable for umlaut aliases because the row failed validation first; on v4 it is not. Reverting the endpoint does not repair duplicates already created — those sit at 7TV. The restore path therefore now filters against the target set the way import does, and import re-checks immediately before the run instead of trusting the snapshot taken when the confirmation dialog opened.

**The check asks 7TV, not our own database — and the first version got that wrong.** It initially read `/api/channels/{name}/emotes`, which serves our Postgres mirror (`ListActiveAsync`, `!IsArchived`). The independent review caught what that breaks: restore is the operation run *because* something already went wrong, most often right after a delete whose closing `sync-deleted` report is still pending, failed, or only partially applied. In exactly that window our database still lists the just-deleted emotes as active while 7TV has already dropped them, so the filter would classify precisely the rows the user wants restored as "already present", hand `startRestore` an empty queue, and report "already in the target set, skipped" — a falsehood at the one moment it matters most. The preflight therefore reads 7TV's own `emoteSet` contents (paginated, public, no token — the write endpoint's *global* rate-limit bucket at 5000/60 s, not the tighter `emote_set_change` one), which is the authoritative answer to the question being asked. A failed fetch is reported as unverified rather than as nothing-to-skip, following the rule already stated next to the same trade-off in `import-target-loader.ts`: a failed check must read as "not verified", never as a false all-clear. That matters more here than usual, because the duplicate it fails to catch cannot be repaired afterwards.

**Two smaller findings from the same review.** The mutual-exclusion check against a concurrently running mutation happened *before* the preflight's asynchronous fetch, so another delete or restore could claim the arbiter while it was in flight — each engine only refuses a second run of its own kind, so an import could have overlapped a delete against the same set. It is now re-checked inside the callback. And the "everything was skipped" notice lived only inside the run dock, which never mounts when the preflight filters every row and the engine refuses the empty queue — so the one outcome the user needed to see was the one that could not be shown. Both notices moved to the transient status surface (`UI-Designsprache.md` §4.5), where they also stop appearing beside a later run's stale details.

**Two things stay open, deliberately.** Between the last check and the individual `addEmote` there remains a window in which another editor can write; without an atomic operation on 7TV's side that cannot be closed. And the delete run still does not abort on missing permission, because only import has an `abortOn` hook — that changes behaviour and belongs in its own ticket. Unlike the duplicate hole, the migration does not make it worse: the failure was a per-row failure before and after.

**The delete path was migrated too, against the second opinion's advice.** Codex Sol objected: DELETE works, the migration risks it for no reason, and a rollback would take the fix with it. Against that: v3 is visibly legacy at 7TV — the alias repair was never backported — two endpoints with two error contracts in one engine cost more code than one, and `removeEmote` had by then already been run live and shown by set comparison to act exactly as intended. As a condition from the review the proof list was widened: import, restore, delete, permission failure and duplicate protection are each verified live, not just the import.
...
`web/src/app/shared/seven-tv/file-import-trigger-gate.ts` ·
`web/src/app/shared/seven-tv/file-import-trigger.ts` ·
`web/src/app/features/usage-stats/usage-stats-page.html` ·
`docs/UI-Designsprache.md` (§7.2, §7.3 — nur die zitierten Wortlaute)

**Der Fall.** Wörtlicher Wunsch des Betreibers: *„mir gefallen die '...' nicht. Ich würde es
besser finden wenn die Buttons ohne ... heißen. also einfach nur Übertragen und Datei einspielen
bzw. statt Datei einspielen 'Importieren'."* Das ist eine Geschmacks-/Stilentscheidung des
Produktinhabers, keine Korrektur eines Fehlers — die frühere Begründung für die Ellipse (Eintrag
weiter unten, „mit Ellipse, weil ein Dialog folgt (wie „Übertragen…")") wird damit **aufgegeben,
nicht widerlegt**. Sie stand zu Recht dort, als sie geschrieben wurde; sie gilt nur nicht mehr.
Ein zweiter, während der Umsetzung nachgereichter Wunsch galt dem Dialogtitel selbst, ebenfalls
wörtlich: *„Das 'Datei Einspielen' klingt einfach nicht gut auf deutsch. dann lieber auch im
Dialog 'Datei Importieren' nennen oder so."*

**Regel: Knöpfe und Menüeinträge tragen keine Ellipse — app-weit, nicht nur die Aktionszeile.**
Ursprünglich nur für den Seitenkopf der Nutzungsseite angefragt, aber während der Umsetzung um
einen Nachtrag erweitert: `dateRange.presetCustom` („Eigener Zeitraum …" / „Custom range …", ein
Menüeintrag im Zeitraum-Popover, kein Knopf im engeren Sinn) verliert die Ellipse aus demselben
Grund. Die Regel ist damit nicht auf einzelne Flächen beschränkt, sondern auf den Elementtyp:
alles, was eine Aktion auslöst oder eine Auswahl trifft. **Ausdrücklich nicht betroffen sind
Fortschritts- und Ladetexte** („Lädt…", „Verbindet…", „Synchronisierung wird angestoßen…") — die
bezeichnen eine laufende Handlung, keinen folgenden Dialog, und behalten ihre Ellipse.

**Die vier Zeichenketten der Aktionszeile.** `import.copyButton` „Übertragen…" → „Übertragen"
(en: „Transfer…" → „Transfer"); `import.dockCopyButton` folgt wortgleich, weil es derselbe Knopf
in seiner Dock-Kurzform ist — dieselbe Aktion, dasselbe Wort, eine stehengebliebene Ellipse dort
wäre ein Widerspruch zum Header-Knopf gewesen. `restore.import.fileLabel` „Datei auswählen…" →
„Datei auswählen" (en: „Choose file…" → „Choose file") verliert nur die Ellipse, der Wortlaut
bleibt — dieser Knopf sitzt **im** Einspiel-Dialog und war nicht Gegenstand einer Umbenennung.

**Die Umbenennungen: `restore.import.trigger` und `restore.import.title`.** Der Trigger „Datei
einspielen…" → „Importieren" (en: „Import file…" → „Import") ist mehr als eine
Ellipse-Entfernung — der Nutzer wollte ausdrücklich das andere Wort. Der Dialogtitel zog mit dem
Nachtrag nach: „Datei einspielen" → „Datei importieren" — Kleinschreibung, nicht „Datei
Importieren" wie in der beiläufigen Nutzeräußerung, weil die deutsche Rechtschreibung und jeder
andere Titel in beiden Locale-Dateien Kleinschreibung nach dem ersten Wort verlangen. Die
englische Seite bleibt hier unverändert: `restore.import.title` war mit „Import file" bereits
stimmig, nur die deutsche Fassung klang schlecht. Trigger und Titel tragen damit bewusst
unterschiedliche Wörter („Importieren" vs. „Datei importieren") — das ist keine Inkonsistenz,
sondern Knopf-Kürze gegen Dialog-Vollständigkeit. Geprüft und bestätigt: „Datei importieren"
bleibt disjunkt vom Bestätigungsdialog-Titel (`{{count}} Emote(s) nach {{channel}} kopieren?`,
Muster `/kopieren\?$/` in `waitForImportConfirmDialog`) — genau diese Überschneidungsfalle war
beim Bau der Suite der Hauptaufwand, und sie bleibt vermieden.

**Die Nachtrag-Zeile: `dateRange.presetCustom`.** „Eigener Zeitraum …" → „Eigener Zeitraum" (en:
„Custom range …" → „Custom range"). Die Ellipse stand hier mit einem Leerzeichen davor
(„Zeitraum …", anders als bei den übrigen vier Zeichenketten) — das Leerzeichen ist mit
weggenommen, sonst bliebe ein Leerzeichen am Zeilenende stehen. **Nicht verwechseln** mit
`usageRange.presetCustom` (Nutzungsbereich-Popover, „eigener Bereich" / „custom range") — der
trug nie eine Ellipse und bleibt unverändert. Der Schlüssel wird nur über `labelKey` konsumiert
(`shared/datetime/date-range-menu.ts:31`), kein Wortlaut steht im Code; ein Test, der den
sichtbaren Text als Anker benutzt, existierte für diesen Schlüssel nicht und musste nicht
nachgezogen werden.

**Testanker.** Acht Fundstellen zitierten den alten Wortlaut als Prüfwert statt nur erklärend:
`file-import-trigger.spec.ts` (der gerenderte `trigger`-Übersetzungswert), `file-import-dialog.spec.ts`
(viermal: `title` und `fileLabel` als Übersetzungs-Fixtures, `toBe`-Erwartungen auf Überschrift und
Picker-Text) und `emote-import.e2e.spec.ts` (fünfmal: die beiden `getByRole`-Locators für Header-
und Dock-Übertragen-Knopf, die Titel-Erwartung des Einspiel-Dialogs, die fokussierte Erwartung auf
„Datei auswählen" und ein `indexOf` auf denselben Text im Dialogtext). `exact: true` trennt
„Übertragen" weiterhin sauber von „Übertragen (2)" — das war schon vor der Änderung so gelöst und
musste nur der neue Wortlaut in den erklärenden Kommentar nachgezogen werden. Reine Erklär-Zitate
...
Diagnosemodus). **Task 6 (Stichtag, fail-closed, Diagnosemodus):** zusätzlich
`src/EmotePurge.Worker/Harness/HarnessOptions.cs` (`SharedChatCutover`),
`src/EmotePurge.Worker/Harness/HarnessCommandLine.cs` (`DiagnosticOption`, `RunHarness.Diagnostic`,
...
**Was die Entscheidung umwerfen würde:** wenn 7TV eine Client-/App-Registrierung für Drittanbieter anbietet. Bis dahin ist jede Wiederaufnahme dieser Frage verlorene Zeit. Der einzige Hebel in unserer Hand wäre, 7TV danach zu fragen (Option D der Untersuchung) — **das ist am 2026-09-01 verworfen worden:** eine öffentliche Anfrage im `SevenTV/SevenTV`-Repo oder im Discord würde EmotePurge gegenüber 7TV sichtbar machen, und das ist derzeit nicht gewollt. Die Option bleibt technisch gangbar und kann jederzeit gezogen werden; sie ist nicht unmöglich, sondern nicht erwünscht. **(Überholt — s. Nachtrag vom 2026-09-10 unmittelbar darunter.)**

**Nachtrag 2026-09-10 — Option D ist gezogen: [`SevenTV/SevenTV#311`](https://github.com/SevenTV/SevenTV/issues/311).** Der technische Befund oben bleibt unverändert gültig; geändert hat sich allein die Haltung zur Sichtbarkeit. Vorausgegangen war eine allgemein gehaltene Frage im 7TV-Discord (ohne Nennung der App), die unbeantwortet blieb — mit `SevenTV/SevenTV#159` als verlinktem Beleg. Genau der trägt aber nichts: **#159 ist geschlossen, vom Autor selbst am Eröffnungstag 2024-12-05**, mit dem Kommentar, man könne Nutzer stattdessen bitten, einen selbst kontrollierten Account als Editor einzutragen — „so I guess this is not needed". Kein Maintainer hat je geantwortet. Wer dem Link folgt, liest also einen zurückgezogenen Wunsch, kein offenes Anliegen; das erklärt das ausbleibende Echo besser als Desinteresse.

**Das Ticket nennt EmotePurge nicht.** Ich-Form, kein Name, kein Link, kein Screenshot — beschrieben ist ein Dritt-Webtool zum Prüfen und Aufräumen von Emote-Sets. Es schildert das Problem samt Kosten (Self-XSS-Gewöhnung, Vollzugriffs-JWT statt begrenztem Token, 30-Tage-Lauf mit stillem Verfall bei jedem 7TV-Logout, Touch-Geräte ganz ohne Weg), belegt es mit dem am 2026-09-10 erneut reproduzierten `403` / `error_code 10000` / `return_to origin mismatch` auf **beiden** Hosts, grenzt sich ausdrücklich gegen #159 ab und bietet drei Lösungsformen an, billigste zuerst: nutzer-erzeugte scoped Tokens nach dem PAT-Muster, eine je registrierter App erweiterbare `return_to`-Allowlist, volle Client-Registrierung. Ausdrücklich **nicht** verlangt werden erhöhte Limits oder Bulk-Zugriff. Erwartungshaltung: zahlreiche offene Tickets in dem Repo stehen seit über einem Jahr bei null Kommentaren — das Ticket ist ein Marker, keine Überholspur.

**Der Editor-Account-Workaround aus #159 ist kategorisch verworfen** („kommt nicht in frage, auf keinen fall"). Er löste den DevTools-Umweg tatsächlich und würde sogar 7TV-Schreibzugriff auf Touch-Geräten ermöglichen — kostet aber genau das, was hier nicht verhandelbar ist: dauerhafte Schreibrechte auf fremden 7TV-Accounts, die Aufgabe von Grundsatz 4 und ein serverseitiges 30-Tage-JWT, das jemand von Hand nachziehen müsste. Nicht erneut vorschlagen, auch nicht als Touch-Lösung.

**Nicht entschieden, nur erwogen:** das Repo in eine eigene GitHub-Organisation zu verschieben, damit das Projekt nicht mehr über das Personenprofil auffindbar ist. Der Umzug schließt nur die Richtung Profil → Projekt; Commit-Autorenschaft und Contributor-Liste bleiben zurückverfolgbar. Drei Posten, falls es je gemacht wird: der Org-Name muss durchgehend kleingeschrieben sein, weil `publish.yml` `github.repository_owner` ungefiltert in den GHCR-Tag setzt; GHCR-Packages wandern beim Repo-Transfer **nicht** mit, sondern werden unter der Org neu angelegt und müssen dort auf öffentlich gestellt werden; und SonarCloud unterstützt keinen Projekt-Transfer zwischen GitHub-Orgs, kostet also `projectKey`, Token und die bisherige Analysehistorie. Für das Quality Gate ist Letzteres folgenlos, weil es `new_coverage` gegen die Zielbranch misst.
