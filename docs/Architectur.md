...
**Return channel (live updates, since 2026-07-31):** Worker and Api publish thin notification events (`{type, channel, sessionId?}` — never data) to the Redis channel `live:events` (contract: `Core/Messaging/LiveEvents.cs`). For this the Api is itself a Redis subscriber for the first time: `RedisLiveEventStream` (Infrastructure, singleton, lazy subscribe on the first client) fans the events out to open **Server-Sent Events** connections (`GET /api/channels/{name}/live`, `GET /api/admin/live` — native `TypedResults.ServerSentEvents`, no SignalR). The browser then refetches through the normal REST endpoints (notify-and-refetch). Because every Api replica subscribes itself, the mechanism works without a backplane and without sticky sessions even with several replicas. Rationale and operating contract (broker heartbeat 15 s, 10-minute connection cap, connection limits instead of a rate limit, proxy requirements) are in the DECISIONS entry of 2026-07-31; since 2026-09-11 the Api additionally writes its own keepalive at least every 5 s so proxies do not time out an idle connection. Prompt release of an abandoned stream's slot does **not** come from that keepalive — production measurement showed the proxy chain (Cloudflare → nginx) holds the slot for 15–25 s regardless of write frequency — but from the SPA explicitly calling `DELETE /api/live/connections/{id}` when it closes a stream on purpose (DECISIONS entry of 2026-09-11 correcting the same-day diagnosis, #128).
...
| `channel.synced` | Worker: `Worker` (JOIN/RESYNC command **unconditional**, boot recovery only on change), `SevenTvPeriodicResyncWorker` and `SevenTvEventClient` (delta plus follow-up/gap-fill resyncs) **only on change** · Api: `SevenTvEndpoints` `POST /api/seventv/emote-sets/{emoteSetId}/sync-deleted` and `.../sync-restored`, per hit channel with ≥1 emote newly archived or newly un-archived (restore-per-set spec 5.4). The channel-bound `EmoteEndpoints` routes of the same name are the legacy Guid form (spec 5.6): they change no row any more, so they publish no live event of their own — only the guarded resync they trigger does, from the worker |
...
**Endpoint → filter mapping** (group filters resolved):
...
- **Backend sync — deviation from the original specification:** The frontend reports deleted/restored ids to the C# API via `POST /api/seventv/emote-sets/{emoteSetId}/sync-deleted`/`.../sync-restored` (set-centric, restore-per-set spec 5.1) — the reported 7TV emote set, not a channel, is the address, and every tracked channel with that set active is written; an untracked or non-active set still leaves an audit trail. It marks the affected `Emote` rows as `IsArchived = true`/`false` (soft archive, no hard delete — see the decision log in CLAUDE.md); the 1-minute `SevenTvPeriodicResyncWorker` remains the actual safety net. **The channel-scoped `POST /api/channels/{channelName}/emotes/sync-deleted`/`.../sync-restored` still exist** (consistent with every other channel-related endpoint, sitting behind `UsageStatsAccessAuthorizationFilter` — not `ChannelManagementAuthorizationFilter`, since 7TV editors of the channel may report as well, see that class's comment), **but only as the legacy Guid-keyed form (E4): it changes no `Emote` row at all any more** — a browser tab open across a 7TV set switch could otherwise archive a row of the *new* active set on the strength of a body that carries no set of its own. It only counts and audits (`legacyBodyForm: true`) and then triggers the same guarded per-channel resync the set-centric routes use, which is what actually reconciles the row against 7TV.
- **Voting UI:** Thumbs up/down per emote (keep/delete), one's own vote is highlighted (`MyVote`, delivered along with the results — since the login requirement of 2026-07-27 always a real user, no anonymous `null` case any more); session creation/ending/deletion is visible only to managers (`ChannelManagementAuthorizationFilter`), the results view sits behind `VoteAudienceFilter` (login + membership of the session's target audience, still visible to the original target audience after the session has ended). **One creation entry point (since 2026-09-19):** "Put up for vote" from the multi-selection of the usage-stats grid (`CreateVoteSessionDialog`, which passes the selection on as a fixed ballot). The voting list's own inline form is gone, but the dynamic whole-set ballot it used to produce (`emoteIds: null`) is still a valid shape in the API contract (`CreateVoteSessionRequest`, `VoteSessionCreateRequest`, `VoteSessionService.CreateAsync`) — it just has no UI surface creating it anymore.
...
| TLS/reverse proxy | none, direct HTTP access to `localhost:8080` | A host-native (not containerised) reverse proxy in front of the loopback port terminates TLS for `emotepurge.app` and sets `X-Forwarded-Proto`/`-For`; `ForwardedHeadersMiddleware` in `Program.cs` trusts it because `172.16.0.0/12` — Docker's default address pool, which the bridge gateway the proxy arrives from belongs to — is in `KnownIPNetworks`, alongside the loopback defaults. Until 2026-09-16 it trusted nothing beyond loopback and dropped both headers silently, see the DECISIONS entry of that date |
